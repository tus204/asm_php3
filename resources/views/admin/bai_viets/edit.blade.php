        ...
        <div class="main-content-wrap">
            <div class="flex items-center flex-wrap justify-between gap20 mb-27">
                ...
                <ul class="breadcrumbs flex items-center flex-wrap justify-start gap10">
                    ...
                    <li>
                        <div class="text-tiny">Update post</div>
                    </li>
                </ul>
            </div>

            <form class="tf-section-2 form-add-product" method="POST" enctype="multipart/form-data"
                action="{{ route('bai_viets.update', $baiViet) }}">
                @csrf
                @method('PUT')
                <div class="wg-box">
                    <fieldset class="name">
                        ...
                        <div class="upload-image flex-grow mb-10">
                            @if ($baiViet->anh_bia)
                                <div class="item" id="imgpreview" style="">
                                    <img src="{{ Storage::url($baiViet->anh_bia) }}" class="effect8" alt="">
                                </div>
                            @endif
                        ...
                        <fieldset class="name">
                            ...
                            <div class="select mb-10">
                                <select class="" name="is_publish">
                                    <option value="1"
                                        {{ old('is_publish', $baiViet->is_publish) == 1 ? 'selected' : '' }}>Yes
                                    </option>
                                    <option value="0"
                                        {{ old('is_publish', $baiViet->is_publish) == 0 ? 'selected' : '' }}>No
                                    </option>
                                </select>
                            </div>
                        </fieldset>
                        ...
                        <fieldset class="name">
                            ...
                            <div class="select mb-10">
                                <select class="" name="is_comment">
                                    <option value="1"
                                        {{ old('is_comment', $baiViet->is_comment) == 1 ? 'selected' : '' }}>Yes
                                    </option>
                                    <option value="0"
                                        {{ old('is_comment', $baiViet->is_comment) == 0 ? 'selected' : '' }}>No
                                    </option>
                                </select>
                            </div>
                        </fieldset>
                ...
                <div class="wg-box">
                    <fieldset class="noi_dung">
                        ...
                        <textarea id="noi_dung" class="mb-10 @error('noi_dung') error-border @enderror " name="noi_dung"
                            placeholder="Description" tabindex="0" aria-required="true">{{ old('noi_dung', $baiViet->noi_dung) }}</textarea>
                    </fieldset>
                    @error('noi_dung')
                        <div class="text-tiny text-danger">{{ $message }}</div>
                    @enderror
                    ...
                    <div class="cols gap10">
                        <button class="tf-button w-full" type="submit">Update Post</button>
                    </div>
                </div>
            </form>
        </div>
